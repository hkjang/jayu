...
> **"자동차가 시원하게 뚫린 고속도로(추세장)를 달릴 때와, 바퀴가 헛돌고 이리저리 미끄러지는 진흙탕(횡보장)을 지날 때 자동으로 다른 기어(전략)를 넣어주는 변속 시스템입니다!"**

*   **쉽게 설명하자면**: 주식 시장은 일방적으로 한 방향으로 달리는 날(추세장)이 있고, 정해진 상자 안에서 오르락내리락 맴돌기만 하는 날(횡보장)이 있습니다. 본 전략은 현재 시장이 어떤 상태인지 속도계(ADX 지표)로 똑똑하게 판정한 뒤, 상태에 맞춰 주 무기를 스위칭합니다.
*   **어떻게 스위칭하나요?**:
    - **고속도로(ADX가 높음)**: 한 방향으로 밟고 나아가는 **"돌파 및 이평선 추격 매매"** 모드로 전환합니다.
    - **진흙탕(ADX가 낮음)**: 제자리 회전이 심하므로 올라타면 손실만 봅니다. 대신 바닥에 닿을 때 사서 천장에 닿을 때 파는 **"눌림목/박스권 역추세 매매"** 모드로 전환합니다.
*   **핵심 철학**: "하나의 무기로 평생 싸우는 것"이 아니라, **"날씨와 지형에 맞게 신속하게 장비를 스왑하여 계좌의 손실을 방어하고 생존율을 올리는"** 기어 변속 시스템입니다.
