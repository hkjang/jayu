...
class TestDantaSimulation(unittest.TestCase):
    ...
    def test_kelly_size(self):
        ...
        self.assertEqual(
            size_normal, 0.20
        )  # (0.6*2 - 0.4)/2 = 0.4 -> half kelly 0.2 -> min(0.2, 0.3) = 0.20
    ...
    def test_meta_decay(self):
        ...
        # 가상의 초기 메타 상태
        meta = {"rsi_lo": {"40": {"wins": 10.0, "total": 20.0}}}
    ...
    def test_market_trend_filter(self):
        ...
        market_trends = {"^SOX": {date: False for date in self.df.index}}
    ...
    def test_migrations(self):
        ...
        old_best = {"SOXL": {"params": {"rsi_lo": 40}, "metrics": {"total_return": 15.0}}}
        ...
        old_pool = {"SOXL": [{"params": {"rsi_lo": 40}, "fitness": 1.5}]}
        ...
        old_history = {"SOXL": [{"date": "2026-06-13", "metrics": {}}]}
    ...
    def test_connors_rsi2_backtest(self):
        ...
        self.assertTrue(
            len(connors_exits) > 0,
            f"Expected connors_exit, but got: {[t['reason'] for t in trades]}",
        )
    ...
    def test_breakeven_stop_backtest(self):
        ...
        self.assertTrue(
            len(be_exits) > 0, f"Expected breakeven exit, but got: {[t['reason'] for t in trades]}"
        )
    ...
    def test_williams_breakout_backtest(self):
        ...
        df_ind.iloc[entry_idx, df_ind.columns.get_loc("Close")] = (
            106.0  # Close > Open + (10 * 0.5) -> 돌파!
        )
        ...
        self.assertTrue(
            len(target_exits) > 0, f"Expected target exit, but got: {[t['reason'] for t in trades]}"
        )
    ...
    def test_atr_target(self):
        ...
        params = {
            ...
            "gap_min": -0.5,
            "stop_pct": 0.05,
            "target_pct": 0.50,  # 고정 익절은 50%로 매우 높게 설정
            "hold_days": 5,
            "use_atr_stop": False,
            "atr_mult_stop": 2.0,
            "trail_stop": False,
    ...
    def test_volume_breakout_backtest(self):
        """21. 거래량 스파이크 및 가격 채널 돌파 진입 시나리오 검증"""
        params = {
        ...
        df_ind = add_indicators(self.df)
        entry_idx = len(df_ind) - 12

        # 진입 조건 조작
        # 1. 200일선 정배열
        df_ind.iloc[entry_idx, df_ind.columns.get_loc("Close")] = 150.0
        df_ind.iloc[entry_idx, df_ind.columns.get_loc("sma200")] = 100.0

        # 2. 거래량 급증 (Volume > volume_ma20 * 2.5)
        df_ind.iloc[entry_idx, df_ind.columns.get_loc("Volume")] = 30_000_000.0
        df_ind.iloc[entry_idx, df_ind.columns.get_loc("volume_ma20")] = 10_000_000.0

        # 3. 가격 돌파 (Close > high_max_10)
        # N=10일 고가 값을 140으로 조작하고, 오늘 Close를 150으로 두어 돌파하게 만듦
        df_ind.iloc[entry_idx, df_ind.columns.get_loc("high_max_10")] = 140.0
        df_ind.iloc[entry_idx, df_ind.columns.get_loc("vol_ratio")] = 2.0
        df_ind.iloc[entry_idx, df_ind.columns.get_loc("gap")] = 0.0

        # 다음 날 (진입일) Open
        df_ind.iloc[entry_idx + 1, df_ind.columns.get_loc("Open")] = 151.0
        # 다다음 날 High 올려서 target 청산 유도
        df_ind.iloc[entry_idx + 2, df_ind.columns.get_loc("High")] = 180.0
        df_ind.iloc[entry_idx + 2, df_ind.columns.get_loc("Low")] = 145.0
